...
@pytest.mark.asyncio
async def test_get_improved_resume(
    client: AsyncClient,
    normal_user_token_headers: dict[str, str],
    db: Session,
    normal_user: User,
    ...
    r = await client.get(
        f"/resume/{resume_via_crud.id}/improve", headers=normal_user_token_headers
    )
    ...
    assert resume_via_crud.id == resume_via_api.id
    assert resume_via_crud.title == resume_via_api.title
    assert resume_via_crud.content + " [Improved]" == resume_via_api.content
